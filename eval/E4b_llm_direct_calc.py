...
2026-09-03 실측(Groq qwen/qwen3.8-27b)에서는 **LLM 이 5건을 전부 정확하게, 재현
가능하게 계산했다**(최대오차 0원, 재현성 5/5). 기대와 다른 결과이므로 그대로
기록한다. 이 결과가 뒤집는 것은 "LLM 은 산수를 틀린다" 는 (우리가 쓰지 않는)
논거이지, "재현이 구조적으로 보장돼야 한다" 는 논거가 아니다 —
docs/평가결과.md 의 E4b 절 참고.
