...
| 순위 | 할 일 | 비고 |
|---|---|---|
| P0 | **배포 URL 확보** | 제출물 중 유일하게 남은 ❌. 이미지는 검증됐으니(`docker compose up` 통과) 호스팅만 정하면 된다 |
| P0 | 배포 환경에 **LLM 백엔드** 설정 (`ANTHROPIC_API_KEY` 또는 `DN_LOCAL_MODEL_PATH`) | 둘 다 없으면 `StubClient` 가 `{}` 를 돌려줘 **문서에서 추출되는 채무가 0건**이 된다(PDF·이미지·데모 문서 경로 3종). "문서 없이 직접 입력"은 LLM 을 쓰지 않아 키 없이도 끝까지 동작하지만, 심사자가 AI 추출을 보려면 백엔드가 있어야 한다 |
