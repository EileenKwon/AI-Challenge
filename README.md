...
2026-09-02 작업으로 배포 전 체크리스트 항목 4개가 닫혔다 — 정책 카드 6종 전량 검증
(`allow_unverified_cards: false`), 레드팀 `attacks.yaml` 전량 통과, 화면 02 데모 문서
경로 연결, 375px 화면 7종 워크스루. 같은 워크스루에서 설명문 결함 1건도 발견해 고쳤다.
...
| Docker 빌드 검증 | ⚠️ 정적 검토로 결함 3건 수정(`fontconfig` 누락 · `.dockerignore` 부재 · 데모 문서가 이미지에서 통째로 제외되던 문제) — **실제 `docker build` 실행 검증은 여전히 못 함**(작업 환경에 Docker 없음) |
...
| 순위 | 할 일 | 비고 |
|---|---|---|
| P0 | **LLM 백엔드 확보 후 배포** (`ANTHROPIC_API_KEY` 또는 `DN_LOCAL_MODEL_PATH`) | 둘 다 없으면 `StubClient` 가 `{}` 를 돌려줘 **추출 채무가 0건**이 된다. 화면은 끝까지 진행되지만 데모 문서를 골라도 결과가 비어 있다 — 배포 URL 을 내기 전 반드시 해결 |
| P0 | Docker 빌드·`docker compose up` 실제 환경에서 실행 | 정적 결함 3건은 고쳤지만 실제 빌드는 아직 아무도 안 돌려봄. Docker 있는 환경에서 1회 실행 필요 |
...
| P2 | 화면 02 "문서 없이 직접 입력" 구현 | 4가지 방식 중 유일하게 남은 미구현 항목 |
...
**"데모용 합성 문서 선택"으로 조회서 없이 체험할 수 있다.** 13종 중 하나를 고르면
브라우저가 `/demo-docs/<케이스>.pdf` 를 받아 일반 업로드 API 로 그대로 보낸다 —
업로드 검증·PII 마스킹·인젝션 스캔을 우회하는 별도 서버 경로를 만들지 않기 위해서다.

> ⚠️ 남은 알려진 제약
> - **LLM 백엔드가 없으면 추출 채무가 0건이다.** `StubClient` 는 `{}` 를 돌려주므로
>   화면 7종은 끝까지 진행되지만 결과가 비어 있다. 데모 문서를 골라도 마찬가지다 —
>   배포에는 `ANTHROPIC_API_KEY` 또는 `DN_LOCAL_MODEL_PATH` 중 하나가 반드시 필요하다.
> - 업로드(02) 화면의 "문서 없이 직접 입력"은 대응하는 API가 아직 없어 안내
>   메시지만 표시한다.
> - 보완입력(04)의 "최근 6개월 이내 신규채무 여부"(`Q5_RECENT_DEBT`) 질문은
>   화면에는 남아 있지만 저장할 도메인 필드가 아직 없어 전송되지 않는다.
>   (`recent_debt_ratio` 는 이 문항이 아니라 채무별 `executed_at` 에서 파생 계산된다.)
...
- [ ] Docker 빌드/`docker compose up` 실제 환경에서 검증 — 정적 결함 3건 수정(`fontconfig` 누락 · `.dockerignore` 부재 · 데모 문서가 이미지에서 제외되던 문제, 2026-09-02). 실제 `docker build` 실행은 여전히 미검증 — Docker 가 있는 환경에서 `docker compose up --build` 한 번 실행 필요
