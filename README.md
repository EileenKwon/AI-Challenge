...
| Docker 빌드 검증 | ✅ 완료(2026-09-02) — **`docker compose build` / `up` 실행 검증.** 컨테이너 `healthy`, `scripts/smoke.sh` 전 구간 통과(요약서 PDF 415KB 생성 = 컨테이너 안에서 WeasyPrint·한글 폰트 동작 확인), `/demo-docs` 도 이미지 안에서 서빙됨. 이 과정에서 **빌드가 실제로 깨져 있던 것을 발견**했다 — 아래 참고 |
