...
배치를 한 줄로 줄이면 **"AI가 읽고, 코드가 계산하고, 공식기관이 판정한다"** 이다. LLM 을 계산·판정에서 배제한 것은 보수적 선택이 아니라, 해당 업무가 조건 단위 설명가능성(신용정보법 제36조의2가 요구하는 "주요기준" 수준)과 재현가능성을 요구하기 때문이다. 사후 해석으로 재구성한 설명은 그 요구를 만족시키지 못한다. 상세는 기획서 6.3.
